...
app.get("/api/dashboard/spending-by-shop", (req, res) => {
  const query = `
    ...
    FROM transactions
    WHERE amount < 0 AND shop NOT IN ('خصم نقدي من الحساب', 'إيداع في الحساب')
    GROUP BY shop
...
app.get("/api/dashboard/participant-spending", (req, res) => {
  const query = `
    SELECT
      p.name,
      SUM(ABS(t.amount)) as total_spent_in_bills
    FROM transactions t
    JOIN participants p ON p.id = t.participant_id
    WHERE t.amount < 0 AND p.deleted = 0 AND t.shop NOT IN ('خصم نقدي من الحساب', 'إيداع في الحساب')
    GROUP BY p.name
    ORDER BY total_spent_in_bills DESC;
  `;
  db.all(query, [], (err, rows) => {
    if (err) return res.status(500).json({ error: err.message });
    res.json(rows.map(row => ({...row, total_spent_in_bills: parseFloat(row.total_spent_in_bills.toFixed(3))})));
  });
});
...
app.get("/api/purchases/names-status", (req, res) => {
  ...
  const query = `
    SELECT
      pn.id,
      pn.name,
      COALESCE(SUM(pt.amount), 0) as total_credit
    FROM
      purchases_names pn
    LEFT JOIN
      purchases_transactions pt ON pn.id = pt.name_id
                               AND pt.type = 'credit'
                               AND pt.date >= ?
    GROUP BY
      pn.id, pn.name
...
app.get("/api/purchases/transactions", (req, res) => {
  ...
  const totalQuery = `
    SELECT 
      SUM(CASE WHEN type = 'credit' THEN amount ELSE -amount END) as totalCash
